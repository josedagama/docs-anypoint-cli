...
|===
|Command |Description
| xref:datagraph-source.adoc#datagraph-sources-add[datagraph sources add] | Adds an API source 
| xref:datagraph-source.adoc#datagraph-sources-promote[datagraph sources promote] | Promotes an API source
| xref:datagraph-source.adoc#datagraph-sources-update[datagraph sources update] | Updates an API source
| xref:datagraph-source.adoc#datagraph-scale[datagraph scale] | Scales API calls made to the unified schema
| xref:datagraph-source.adoc#datagraph-schema-download[datagraph schema-download] | Downloads the unified schema
| xref:datagraph-source.adoc#datagraph-sources-delete[datagraph sources delete] | Deletes an API source
| xref:datagraph-source.adoc#datagraph-sources-get[datagraph sources get] | Gets an API source
| xref:datagraph-source.adoc#datagraph-sources-list[datagraph sources list] | Lists all API sources
| xref:datagraph-source.adoc#datagraph-describe[datagraph describe] | Returns information about DataGraph
|===
...
[[rtf-apps]]
== Runtime Fabric Application Management 

[%header,cols="35a,65a"]
|===
|Command |Description
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-delete[runtime-mgr application delete] | Deletes an application
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-deploy[runtime-mgr application deploy] | Deploys an application to a specified target using an Exchange application and runs the specified runtime version
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-describe[runtime-mgr application describe] | Describes an application
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-describe-json[runtime-mgr application describe-json] | Describes an application in a raw JSON response
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-download-logs[runtime-mgr application download-logs] | Downloads logs of an application from a specification to a specified directory 
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-list[runtime-mgr application list] | Lists all applications in an organization
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-modify[runtime-mgr application modify] | Modifies a deployed application
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-start[runtime-mgr application start] | Starts a stopped application
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-stop[runtime-mgr application stop] | Stops running an application
| xref:runtime-fabric-apps.adoc#runtime-mgr-application-tail-logs[runtime-mgr application tail-logs] | Tails an application's logs from a specification 
|===
